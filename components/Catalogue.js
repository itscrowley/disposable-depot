...
// --- OPTIMIZATION LOGIC (With Magic Trim) ---
const optimizeImage = (url, isLightbox = false) => {
  if (!url) return '/images/default.jpg';
  
  // Agar Cloudinary link nahi hai, to waise hi return karo
  ...
  if (isLightbox) {
    // Lightbox: Full Quality, Sharpen, aur Trim (Faaltu space hatao)
    params = 'f_auto,q_auto:best,w_1600,e_sharpen:50,e_trim';
  } else {
    // Grid/Card: Square (800x800), White Background, aur Trim (Product bada dikhega)
    ...
    fetch(`${SHEET_URL}&t=${timestamp}`, { 
      ...
      .then((csvText) => {
        ...
        const products = rows.map((row) => {
          // CSV Parsing fix for commas inside quotes
  ...
  return (
    <>
      <style jsx global>{`
        ...
        /* --- CSS FIX FOR SQUARE IMAGES --- */
        .img-container { 
          background-color: #ffffff;
          border-bottom: 1px solid var(--card-border); 
          position: relative; 
          width: 100%;           /* Full width */
          aspect-ratio: 1 / 1;   /* Force Perfect Square Shape */
          overflow: hidden; 
          padding: 10px; 
        ...
        /* Removed fixed heights to let aspect-ratio work */
